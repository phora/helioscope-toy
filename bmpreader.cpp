...
GLubyte* BMPReader::load(const char *filename, GLuint & width, GLuint & height) {
	...
	for (int row = 0; row < numRows; row++) {
		for (int col = 0; col < numCols; col++) {
			...
			if (bpp == 32) {
			pixelData[idx] = pixBytes[1]; // Red
			pixelData[idx+1] = pixBytes[2]; // Green
			pixelData[idx+2] = pixBytes[3]; // Blue
			pixelData[idx+3] = pixBytes[0]; // Alpha

			//printf("%d, %d, %d, %d\n",pixBytes[0],pixBytes[1],pixBytes[2],pixBytes[3]);
			}
			else {
